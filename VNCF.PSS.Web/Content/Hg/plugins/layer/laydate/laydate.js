...
function(a) {
    ...
    c.initDate = function() {
    	...
    	if(e.length==1){
    		if(e[0].length==8){
    			tempArr = e[0].split('');
    			e[0] = tempArr[0] + tempArr[1] + tempArr[2] + tempArr[3];
    			e[1] = tempArr[4] + tempArr[5];
    			e[2] = tempArr[6] + tempArr[7];
    		}
    	}
    ...
    c.creation = function(a, b,swOpenFlag) {
    ...
    c.events = function() {
        ...
        function() {       	
            var dd = new Date();
            c.valid && c.creation([dd.getFullYear(), dd.getMonth() + 1, dd.getDate()])
//          c.elem[h.elemv] = laydate.now(0, c.options.format),
//          (c.close(), "function" == typeof c.options.choose && c.options.choose(f))
        }),
        h.ok = b("#laydate_ok"),
        c.on(h.ok, "click",
        function() {
        ...
        function(b) {
            ...
            var d = b.keyCode;
            var swOpenFlag = true;
            if(b.srcElement && b.srcElement.id && $("#"+b.srcElement.id).attr("laydateDefault")=="no"){
            	swOpenFlag =false;
            }
            13 === d && c.creation([c.ymd[0], c.ymd[1] + 1, c.ymd[2]],'',swOpenFlag);
            9 === d && c.creation([c.ymd[0], c.ymd[1] + 1, c.ymd[2]],'',swOpenFlag);
        })
    },
    c.init = /*function() {
        c.use("need"),

...
//            var html = '<div class="autocompleter '+opts.customClass.join(' ')+'" id="autocompleter-'+(guid+1)+'">';
            ...
            //新增向上展示列表功能 peiyang
            var html = '';
            if(typeof(opts.dropUp)!=="undefined"){
            	if(opts.dropUp){
            		html = '<div class="autocompleter dropUp'+opts.customClass.join(' ')+'"';
            	}else{
            		html = '<div class="autocompleter '+opts.customClass.join(' ')+'"';
            	}
            }else{
            	html = '<div class="autocompleter '+opts.customClass.join(' ')+'"';
            }
                ...
                if(opts.style){
                	html+='style="'+opts.style+'"';
                }            
                html+=' id="autocompleter-'+(guid+1)+'">';
                if (opts.hint) {
            ...
            data.$node.on("keyup.autocompleter", data, _onKeyup)
                      ...
                      .on("compositionstart.autocompleter", data, _chntypestart)
                      .on("compositionend.autocompleter", data, _chntypeend);
        }
    }

    function _chntypestart(){
    	chntypeflag="1";
    	//console.log("-------中文输入开始");
    }
    
    function _chntypeend(){
    	chntypeflag="0";
    	//console.log("-------中文输入结束");
    }
    /**
            ...
            for (var i = 0; i < 2; i++) {
                for (var item in source) {
                    if (response.length < limit) {
                        switch (i) {
                            case 0:
                            	if(source[item]!=undefined &&source[item].label!=undefined){ //added by cyd for undefined 20180322
...
//        console.log(chntypeflag);chntypeflag=="1"中文输入法开启则不执行查询
       
        if (!data.empty && data.query.length === 0||chntypeflag=="1") {
            _clear(data);
            return;
        } else {
            if (typeof data.source === 'object') {
                ...
                /*Begin:added for 水运舱单支持保存功能 by chenyongda 20190121*/
                }else if(data.saveFlag){//支持保存功能，
                    //将隐藏域内容更新
                    $("#"+data.hiddenId).val(data.query);
                }else{
                /*End :added for 水运舱单支持保存功能 by chenyongda 20190121*/
                	layer.msg('无匹配数据', {icon: 5,time:500});
                  data.$node.attr('pcode',"");
                  //如果设置了隐藏输入框ID则将code赋值
    			  data.$node.val("");
                  if(!data.hiddenId==""){
                  	$("#"+data.hiddenId).val("");
                  }
    ...
    function _buildList(list, data) {
        ...
        var hiddenValue="";
        if(!data.hiddenId==""){
            hiddenValue = $("#"+data.hiddenId).val();
          }
        for (var item = 0, count = list.length; item < count; item++) {
            var classes = ["autocompleter-item"];
            //默认选择第一行与实时更新输入框，这2个参数不兼容
...
//            }
            ...
            if(hiddenValue && value==hiddenValue && label==(hiddenValue+"-"+data.$node[0].value)){
                classes.push("autocompleter-item-selected");
           }
            ...
            if (value) {
            	//新增title鼠标悬停提示
            	if(data.tipsData.length>0){
            		var fg=0;//匹配标志
            		for(var p in data.tipsData){
            				if(data.tipsData[p].tipscode==value){
            					menu += '<li title="'+data.tipsData[p].tipstitle+'" data-value="'+value+'" data-label="'+clear+'" class="'+classes.join(' ')+'">'+label+'</li>';
            					fg=1;
            				}
            			}
            		if(fg==0){//如果一直未匹配到提示，则不输出title属性
        ...
        //新增向上展示列表功能 peiyang
        if(data.$autocompleter.hasClass("dropUp")){
        	//alert(data.$autocompleter.height());
        	var height="400";
        	if(data.$autocompleter.height()>400){
        		data.$autocompleter.css("top","-400px");	
        	}else{
        		data.$autocompleter.css("top","-"+data.$autocompleter.height()+"px");
        	}      	 
        }
        
        if(data.$autocompleter.find(".autocompleter-list").length>0 && data.$autocompleter.find(".autocompleter-item-selected").length==0){
            (data.$autocompleter.find(".autocompleter-list").find(".autocompleter-item")[0]).className=" autocompleter-item autocompleter-item-selected";
        }
    ...
    function _onBlur(e, internal) {
        ...
        if (!internal) {
          ...
          //如果autocompleter处于open状态，且光标移除控件，则清空input和隐藏框的值
            if(data.$autocompleter.hasClass('autocompleter-show')){
            	if(data.$node.val()!==""){
            		//如果input内容不为空则默认选择第一行
            		_setValue(data);
          ...
          //如果input内容为空，则清除数据
            if(data.$node.val()==""){
            	...
            	data.$node.attr('pcode',"");
            	$("#"+data.hiddenId).val("");
            	data.callback.call(data.$autocompleter, data.$node.val(), -1, {value:$("#"+data.hiddenId).val(),label:''});
            }
            
            //当使用input框默认情况按钮时，需清空code和pcode
//            if(data.$node.val()==""){
//            	data.$node.attr('pcode',"")
//                //如果设置了隐藏输入框ID则将code赋值
//                if(!data.hiddenId==""){
...
//            data.$autocompleter.on("mousedown",function(e){
//            	alert(1);
//            });
//            alert(1);
...
//            var value = data.$selected.attr('data-value') ? data.$selected.attr('data-value') : data.$selected.attr('data-label');
           //设置input文字
            var datalabel =data.$selected.attr('data-label');
            var value = "";
            var code = "";
            //默认按_分割或-分割，当传入自定义分隔符时，特殊处理
            if(data.spchar){
            	code= datalabel.split(data.spchar)[0];
                value= datalabel.split(data.spchar)[1];
            }else{
                if(datalabel.indexOf('_')>0){//先按_分割
                	code= datalabel.split('_')[0];
                    value= datalabel.split('_')[1];
                }else{
                     if(datalabel.indexOf('-')>0){//再按-分割                	
                    	code = datalabel.substr(0,datalabel.indexOf("-"));
                    	value= datalabel.substr(datalabel.indexOf("-")+1);
    /*             		code= datalabel.split('-')[0];
                        value= datalabel.split('-')[1];*/
                 	}else{
                     	code= datalabel;
                        value= datalabel;
                 	}
            ...
            data.$node.val(value);
            data.$node.attr('pcode',code)
            //如果设置了隐藏输入框ID则将code赋值
            if(!data.hiddenId==""){
            	$("#"+data.hiddenId).val(code);
            }

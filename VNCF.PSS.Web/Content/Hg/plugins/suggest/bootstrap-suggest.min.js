...
function(a) {
	...
	var b = {
		init: function(b) {
			function c(b, c, d) {
				...
				setTimeout(function() {
					if(c.is(":visible")&&c.find("." + s.listHoverCSS).length==0){						
						...
						if(s.showAllFlag){
							if(c.context.value){
								currentList = c.find("tr:contains("+c.context.value+")");	
							}							
						}
			...
			function h(b, c, d, e) {
				...
				if("url"==e.getDataMethod){
					//%号做解码时存在BUG，需要进行替换
					b = encodeURI(encodeURI(b.replace(new RegExp("%","gm"),"%25")));
				}
				...
				if (b = b || "", e = e || s, e.url) m = -1 !== e.url.indexOf("?") ? "&" : "?", n = e.jsonp ? [e.url + b, m, e.jsonp, "=?"].join("") : e.url + (("url"==e.getDataMethod) ? b : ""), a.ajax({
						...
						for (o = f.value.length, h = 0; o > h; h++) 
							for (i in f.value[h]) 
								if (a.trim(f.value[h][i]) && (l(i, e) || k(i, e)) && (-1 !== f.value[h][i].toString().indexOf(b) )) {
									...
									//新增逻辑：模糊匹配时，必须从前往后模糊搜索匹配，所以待搜索项必须从头匹配输入项（页面缓存情况）
									//if(0 == f.value[h][i].toString().indexOf(b)){
									//	p.value.push(f.value[h]);
									//	break
									//}
									
									//修改页面缓存情况匹配逻辑，从头匹配输入项改为前后均可匹配
									if(f.value[h][i].toString().indexOf(b) > -1){
										p.value.push(f.value[h]);
										break;
									}
									//非页面缓存情况，由后台SQL进行like返回时处理待选项解决。
					...
					if(0==p.value.length && ! s.isEditable){
						c[0].value="";
						//如果搜索不到结果，则造一个长度为0数组，通知控件清除键盘记录
						var tempD = {value : []};
						c.trigger("onDataRequestSuccess", tempD);
					}
					d(c, p, e)
				}
		...
		enable: function() {
			return a(this).data("bsSuggest") ? void a(this).attr("disabled", !1).attr("readonly", !1).parent().find(".input-group-btn>.btn").removeClass("disabled").attr("disabled", !1) : !1
		},
		destroy: function() {

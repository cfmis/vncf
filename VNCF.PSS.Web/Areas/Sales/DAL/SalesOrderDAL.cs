...
namespace VNCF.PSS.Web.Areas.Sales.DAL
{
    public class SalesOrderDAL
    {
        ...
        //返回基礎資料,供下拉列表框使用
        //前端是以ID值存儲至數據庫
        public static List<baseUnit> GetBaseInfoReturnList(string strLanguage, string strTableName)
        {
            string strSql = "";
            string strFieldName = "";
            if (strLanguage == "0")
            ...
            DataTable dt = SQLHelper.ExecuteSqlReturnDataTable(strSql);
            List<baseUnit> lstBase = new List<baseUnit>();
            for (int i = 0; i < dt.Rows.Count; i++)
            { 
                baseUnit objModel = new baseUnit();
                objModel.id = dt.Rows[i]["id"].ToString();
        ...
        //返回基礎資料,供下拉列表框使用
        //前端是以Name值存儲至數據庫
        public static List<baseUnit> GetBaseInfoByNameReturnList(string strLanguage, string strTableName)
        {
            string strSql = "";
            switch (strTableName)
            {
